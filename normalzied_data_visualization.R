...
#days_range <-seq(1,1307)   # numeric counter for time (days)
...
# colnames(stays.df[,-1]) <- days_range     # rename column names with date
# rownames(stays.df) <- districtNumber
stays_long <- melt(stays.df, id.vars = "districtNumber")   # melt matrix into long form
stays_long$variable <- as.Date(stays_long$variable, format="%Y/%m/%d")  # convert date from factor to date form
...
## create bar chart
barplot(stays[,1],names.arg = districtNumber, xlab="locations", ylab="Normalized number of stays (%)") # plots a barchart looking at number of stays for each location on a given day
barplot(stays[1,], xlab="date", ylab="Normalized number of stays (%)") # plots a barchart looking at number of stays over time for a given location

# create line plot to compare all cities
ggplot2(stays_long.df, aes(x=date,y))
